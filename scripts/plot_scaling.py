#!/usr/bin/env python3
"""Scaling curves with power-law extrapolation.

Solid line + filled markers = measured. Dashed line + open markers = power-law
projection. Power law: AUC(N) = AUC_inf - A * N^(-alpha), fit per strategy.
...
def fit_strategy(sizes: np.ndarray, aucs: np.ndarray):
    """Fit AUC(N) = AUC_inf - A * N^(-alpha) on all measured points.

    Weight residuals by sqrt(N) so each decade of N contributes ~equally to the
    fit (otherwise low-N points dominate by sheer count). Returns (auc_inf, A, alpha).
    """
    p0 = (aucs.max() + 0.005, 0.5, 0.25)
    bounds = ([aucs.max(), 1e-4, 1e-3],
              [1.0,         50.0,  2.0])
    sigma = 1.0 / np.sqrt(sizes / sizes.min())  # smaller sigma = larger N = more weight
    popt, _ = curve_fit(power_law, sizes, aucs, p0=p0, bounds=bounds,
...
def main():
    ...
    for strategy, (label, color, marker) in STRATEGY_DISPLAY.items():
        ...
        # --- Solid line through measured points (faithful to data) ---
        ax.plot(x_meas, y_meas, color=color, lw=1.6, marker=marker,
                markersize=5, markeredgecolor="white", markeredgewidth=0.5,
                label=label, zorder=4)

        # --- Dashed power-law extrapolation past the last measured point ---
        if auc_inf is not None:
            projected_sizes = np.array([s for s in SIZES_ALL if s > x_meas.max()])
            if len(projected_sizes):
                # Anchor: shift projection so it passes through the last measured point.
                anchor_offset = y_meas[-1] - power_law(x_meas.max(), auc_inf, A, alpha)
                x_dash = np.logspace(np.log10(x_meas.max()),
                                     np.log10(projected_sizes.max() * 1.05), 80)
                y_dash = power_law(x_dash, auc_inf, A, alpha) + anchor_offset
                ax.plot(x_dash, y_dash, color=color, lw=1.4,
                        linestyle=(0, (5, 4)), alpha=0.85, zorder=3)
                y_proj = power_law(projected_sizes, auc_inf, A, alpha) + anchor_offset

...
control separates. Pretraining loss, separately, is not comparable across label
spaces: which arm learned more is not decidable from ours.
\end{abstract}
...
The \emph{label space} has been ablated far less. \citet{pecm} compare a
12-class against a 41-label pretraining target at fixed architecture and data,
but their coarse and fine targets are different objectives rather than rungs of
one contraction; no nested, examples-matched vocabulary ladder exists in jet
physics. Yet if supervised classification is the best pretraining objective, the
vocabulary of that classification is the design choice that matters most, and it
is set by convention --- production taggers have grown from tens to hundreds of
classes without a controlled measurement of what that buys. The vision
literature disagrees with itself about the answer: \citet{huh} found \emph{coarsening}
...
We contribute a controlled instrument for that question in jet physics. The
vision studies hold their image set fixed; we hold fixed a flat
$(p_T, m_\mathrm{SD})$ \emph{reweighted} stream whose acceptance histograms are
byte-identical across arms, so the label change cannot reweight the draw.
\citet{mahajan} showed the best pretraining vocabulary depends on the target; we
ask what one destroys on targets it never names.

\section{A controlled granularity ladder}

\paragraph{Data and vocabulary.} JetClass-II \citep{sophon} labels each jet with one of
...
\midrule
mean (linear) & \meanRes & \meanQcd & \meanTop \\
factor in $1-\mathrm{AUC}$ & $\facRes\times$ & $\facQcd\times$ & $\facTop\times$ \\
\midrule
mean (MLP) & \meanResMlp & \meanQcdMlp & \meanTopMlp \\
factor in $1-\mathrm{AUC}$ & $\facResMlp\times$ & $\facQcdMlp\times$ & $\facTopMlp\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
designed this column as a null-versus-null control and expected parity --- a
prior that cut against the routine transfer of flavour taggers across jet
populations, and was wrong. L162 is better by $\facQcd\times$, an excess of
...
labels transfers to QCD jets and that collapsing it destroys that. A second is
generic: at $K=17$ the representation may compress \emph{any} axis interior to a
group, in which case the excess measures ``unlabelled versus labelled'' and
nothing about flavour. Both flavour axes lie interior to an R16\_Q1 group and the
topology axis does not, so these columns cannot separate the accounts; the
control in Section~4 can. Either way the observation stands, and what a
...
\paragraph{A nonlinear probe recovers part of it, not most of it.} The MLP
reduces R16\_Q1's $1-\mathrm{AUC}$ by \mlpRecovery{} on the merged axis and
\mlpRecoveryQcd{} on the unlabelled one, against \mlpRecoveryL{} for L162, so
some of the lost information is present but nonlinearly encoded. It closes
neither gap and changes no ordering (Table~\ref{tab:probe}): the two excesses
fall to \didMlpMean{} and \didMlpQcd{}. Since a linear probe only lower-bounds
the mutual information, that residual is what distinguishes degradation from a
...
one partition of the data, so our arms' final training losses (\LossLsixtwo{}
for L162, \LossRsixteen{} for R16\_Q1, one seed each) are not the same quantity,
and their ordering
...
decidable from one loss per arm. Scaling laws mapping loss to downstream
performance \citep{scaling} are fitted at a fixed vocabulary; the hazard is on
the reading side.

\paragraph{Pipeline validation.} On JetClass-I under an identical recipe our
arm reaches macro AUC \ePartOurs{} against the published \ePartPub{}
\citep{part}.
...
coarse labels span, we ask what it \emph{destroys} elsewhere --- but locating the
boundary needs a curve.

\paragraph{One transfer axis.} The QCD result rests on a single pair of native
labels, and we have not shown it generalises to other unlabelled distinctions or
